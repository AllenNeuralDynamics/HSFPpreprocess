...
for ROIii in Roi2Vis:
    ...
    PSTHplot(Psth_G_CSall_base[:,ROIii,preR_CS3UR].T, "m", "darkmagenta", "R-")
    plt.axvspan(0, 1.0, color = [1, 0, 1, 0.4])
    plt.axvspan(2.0, 2.5, color = [0, 0, 1, 0.4])
    plt.grid(True)
    plt.ylim([ymin[ROIii],ymax[ROIii]])
    
    plt.subplot(3,2,6)
    PSTHplot(Psth_G_CSall_base[:,ROIii,preUR_CS3R].T, "g", "darkgreen",[])
    PSTHplot(Psth_G_CSall_base[:,ROIii,preUR_CS3UR].T, "m", "darkmagenta",[])
    plt.axvspan(0, 1.0, color = [1, 0, 1, 0.4])
    plt.axvspan(2.0, 2.5, color = [0, 0, 1, 0.4])
    plt.grid(True)
    plt.ylim([ymin[ROIii],ymax[ROIii]])
    ...
    TimeFromPR = TimeFromPR/20 # in seconds
